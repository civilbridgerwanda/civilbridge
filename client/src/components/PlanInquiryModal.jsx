...
export default function PlanInquiryModal({ plan, onClose }) {
  ...
  async function handleSubmit(e) {
    ...
    setError(null);
    try {
      await api.submitPlanInquiry(plan.id, form);
      setSubmitted(true);
    } catch (err) {
  ...
  return (
    <Modal title="Talk to an Expert" onClose={onClose}>
      <p className="mb-4 text-sm text-slate-500">
        About <strong>{plan.title}</strong>. Fill this in and a CivilBridge expert will reach out.
      </p>

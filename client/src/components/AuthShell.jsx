...
export default function AuthShell({
  ...
  return (
    <div className="flex min-h-screen flex-col">
      {/* Lightweight header - same bare logo + wordmark lockup as the main
          site Navbar (no boxed/colored backdrop), so this is the one logo
          treatment used everywhere instead of a bespoke auth-only version. */}
      <header className="border-b border-slate-200 bg-white px-6 py-4">
        <Link to="/" className="flex items-center gap-2 text-xl font-bold text-brand-500">
          <img src={logo} alt="CivilBridge" width="32" height="32" className="h-8 w-8" />
          CivilBridge
        </Link>
      </header>
